...
for window_size in range(stride, 5000 + stride, stride):
    hot_ttid_page = 0
    hot_ttfd_page = 0
    warm_page = 0
    cold_page = 0
...
for window_size in range(stride, 5000 + stride, stride):
    ...
    cold_page = 0
    not_swapout_page = 0
    bg_lru_pfn_numbers_window = bg_lru_pfn_numbers[:window_size]
    for pfn in bg_lru_pfn_numbers_window:
        if pfn in swapout_ttid_pfn_numbers_matched:
            hot_ttid_page += 1
        elif pfn in swapout_ttfd_pfn_numbers_matched:
            hot_ttfd_page += 1
            ...
            cold_page += 1
    not_swapout_page = len(bg_lru_pfn_numbers_window) - hot_ttid_page - hot_ttfd_page - warm_page - cold_page
    print(f"{window_size} {hot_ttid_page} {hot_ttfd_page} {warm_page} {cold_page} {not_swapout_page}")
